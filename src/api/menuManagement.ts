...
import { adaptMsg, getEnvelope, unwrap } from '/@/utils/bladeAdapter'
...
const mapTreeNodes = (nodes: any[] = []): any[] =>
  nodes.map((n) => {
    ...
    return {
      ...
      name: title,
      title,
      // grant-tree 原生无 code/alias，若上游已 enrich 则保留
      code: n.code || '',
      alias: n.alias || '',
      path: n.path || '',
      children,
...
/** grant-tree 只有 id/title，用菜单列表补齐 code/alias/path，供「列表权限」按菜单过滤 */
const enrichTreeWithMenuMeta = (tree: any[], menuList: any[]): any[] => {
  ...
  const walk = (nodes: any[]): any[] =>
    (nodes || []).map((n) => {
      const full = byId.get(String(n.id))
      return {
        ...n,
        code: full?.code || n.code || '',
        alias: full?.alias || n.alias || '',
        path: full?.path || n.path || '',
        // 去掉「名称·code」后缀，便于按中文名匹配列表注册表
        menuName: String(full?.name || n.title || n.label || '')
          .replace(/·[A-Za-z0-9_]+$/, '')
          .trim(),
        children: n.children?.length ? walk(n.children) : n.children,
      }
    })
  return walk(tree)
}
...
/**
 * 权限分配菜单树：仅使用后端库表节点（数字 id）
 * 禁止回退到前端路由树（code 字符串 id 与 role-tree-keys 数字 id 对不上，会导致勾选/授权错乱）
 */
export async function getGrantTree() {
  let lastError = ''

  // 1) 官方 grant-tree（含 menu / dataScope / apiScope）
  try {
    const res: any = await request({
    ...
    if (Array.isArray(menu) && menu.length) {
      let menuTree = mapTreeNodes(menu)
      try {
        const listRes: any = await getBackendList()
        menuTree = enrichTreeWithMenuMeta(menuTree, listRes?.data?.list || [])
      } catch {
        /* 无 list 权限时仍返回基础树 */
      }
      return {
        ...
        data: {
          menu: menuTree,
          dataScope: mapTreeNodes(Array.isArray(payload?.dataScope) ? payload.dataScope : []),
          apiScope: mapTreeNodes(Array.isArray(payload?.apiScope) ? payload.apiScope : []),
        },
      }
    }
    lastError = envelope?.msg || 'grant-tree 返回空菜单'
  } catch (e: any) {
    lastError = e?.message || e?.msg || 'grant-tree 请求失败'
  }

  // 2) 回退：/menu/list（数字 id，与 role-tree-keys 一致）
  try {
    const listRes: any = await getBackendList()
    ...
    lastError = e?.message || e?.msg || lastError || 'menu/list 请求失败'
  }
  ...
  try {
    ...
    lastError = e?.message || e?.msg || lastError || 'menu/tree 请求失败'
  }

  throw new Error(
    lastError
      ? `加载权限树失败：${lastError}。请确认当前角色具备「菜单管理/角色管理」权限，且 blade_menu 已同步`
      : '加载权限树失败：库表暂无菜单，请先在菜单管理导入'
  )
}
 ...
 * loaded=false 表示接口失败（勿当成「未授权」空数组去保存，否则会清空角色菜单）
 */
export async function getRoleMenuTree(roleIds: string) {
  if (!roleIds) {
    return {
      ...
      loaded: true,
      msg: '操作成功',
      data: { menu: [] as string[], dataScope: [] as string[], apiScope: [] as string[] },
    }
  }
  try {
    const res: any = await request({
    ...
    if (envelope?.success === false) {
      return {
        code: envelope?.code ?? 500,
        success: false,
        loaded: false,
        msg: envelope?.msg || '加载角色已授权菜单失败',
        data: { menu: [] as string[], dataScope: [] as string[], apiScope: [] as string[] },
      }
    }
    ...
    return {
      ...
      msg: envelope?.msg || '操作成功',
      data: {
        menu: menu.map(String),
        dataScope: (Array.isArray(payload?.dataScope) ? payload.dataScope : []).map(String),
        apiScope: (Array.isArray(payload?.apiScope) ? payload.apiScope : []).map(String),
      },
    }
  } catch (e: any) {
    return {
      ...
      success: false,
      loaded: false,
      msg: e?.message || e?.msg || '加载角色已授权菜单失败',
      data: { menu: [] as string[], dataScope: [] as string[], apiScope: [] as string[] },
    }
  }
}
...
export function getRoutes(roleId?: string, topMenuId?: string) {
  ...
  if (topMenuId !== undefined && topMenuId !== null && topMenuId !== '') params.topMenuId = topMenuId
  return request({
    url: '/api/blade-system/menu/routes',
    method: 'get',
    params,

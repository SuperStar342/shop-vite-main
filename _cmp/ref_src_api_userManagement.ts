...
import { adaptMsg, adaptPage, toBladePage } from '/@/utils/bladeAdapter'
...
export async function uploadAvatar(file: File) {
  ...
  const url = data?.link || data?.url || data?.domainUrl || ''
  if (!url) {
    throw new Error(res?.msg || '头像上传失败，未返回文件地址（请检查 MinIO 桶 bladex 是否已创建）')
  }
  return {
    url: String(url),

<template>
  <div class="oss-management-container auto-height-container">
    <vab-query-form>
      ...
      <vab-query-form-right-panel :span="12">
        <el-form inline :model="queryForm" @submit.prevent>
          <el-form-item>
            <el-input v-model.trim="queryForm.ossCode" clearable placeholder="请输入资源编号" />
          </el-form-item>
          <el-form-item>
            <el-select v-model="queryForm.category" clearable placeholder="请选择存储类型">
    ...
    <el-table ref="tableRef" v-loading="listLoading" border :data="list" row-key="id" @selection-change="setSelectRows">
      ...
      <el-table-column align="center" label="资源编号" min-width="120" prop="ossCode" show-overflow-tooltip />
      <el-table-column align="center" label="存储类型" min-width="120">
        <template #default="{ row }">
          <el-tag type="info">{{ row.categoryLabel || '-' }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column align="center" label="资源地址" min-width="200" prop="endpoint" show-overflow-tooltip />
      <el-table-column align="center" label="外网地址" min-width="200" prop="transformEndpoint" show-overflow-tooltip />
      <el-table-column align="center" label="空间名" min-width="120" prop="bucketName" show-overflow-tooltip />
    ...
    />
    <oss-edit ref="editRef" @fetch-data="fetchData" />
  </div>
</template>
...
import { doOssDelete, getOssList } from '/@/api/resource'
import { $baseMessage } from '/@/hooks'
import OssEdit from './vabAutoComponents/OssEdit.vue'
...
const runDelete = async (ids: string) => {
  ...
  const { msg, success }: any = await doOssDelete({ ids })
  if (success === false) {
...
const fetchData = async () => {
  ...
  try {
    const { data } = await getOssList(queryForm)
    list.value = data.list || []
    total.value = data.total
  } catch (e: any) {
    list.value = []
    total.value = 0
    $baseMessage(e?.message || e?.msg || '加载对象存储失败', 'error', 'hey')
  } finally {

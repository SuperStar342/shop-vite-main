<template>
  <div class="data-scope-container auto-height-container">
    <vab-query-form>
      <vab-query-form-left-panel :span="12">
        <el-button :icon="Plus" type="primary" @click="handleAdd">添加数据权限</el-button>
    ...
    <el-table ref="tableRef" v-loading="listLoading" border :data="list" row-key="id" @selection-change="setSelectRows">
      ...
      <el-table-column v-if="visible('resourceCode')" align="center" label="权限编号" min-width="140" prop="resourceCode" show-overflow-tooltip />
      <el-table-column v-if="visible('scopeColumn')" align="center" label="权限字段" min-width="120" prop="scopeColumn" show-overflow-tooltip />
      <el-table-column align="center" label="规则类型" min-width="120">
        <template #default="{ row }">
          <el-tag effect="plain">{{ row.scopeTypeName || scopeTypeLabel(row.scopeType) }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column v-if="visible('scopeField')" align="center" label="可见字段" min-width="120" prop="scopeField" show-overflow-tooltip />
      ...
      <template #empty>
        <el-empty class="vab-data-empty" description="暂无数据权限规则" />
      </template>
    </el-table>
    ...
    <data-scope-edit ref="editRef" :scope-type-options="scopeTypeOptions" @fetch-data="fetchData" />
  </div>
</template>
...
import { doDeleteDataScope, getDataScopeList, getScopeTypeDict } from '/@/api/permission'
...
const { visible } = useListColumns('DataScope')
...
const runDelete = async (ids: string) => {
  ...
  const { msg, success }: any = await doDeleteDataScope({ ids })
...
const handleRowDelete = (row: any) => {
  ...
  ElMessageBox.confirm('确定删除该数据权限？', '提示', { type: 'warning' })
    .then(async () => {
      try {
        await runDelete(id)
      } catch (e: any) {
...
const handleBatchDelete = () => {
  ...
  ElMessageBox.confirm('确定删除选中数据权限？', '提示', { type: 'warning' })
    .then(async () => {
      try {
        await runDelete(ids)
      } catch (e: any) {
...
const fetchData = async () => {
  ...
  try {
    const { data } = await getDataScopeList(queryForm)
...
onBeforeMount(async () => {
  try {
    scopeTypeOptions.value = await getScopeTypeDict('data_scope_type')
  } catch {

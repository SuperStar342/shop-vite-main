  ...
  <login-container v-slot="{ alignType }">
    <div class="login-container">
      <div class="login-form" :class="`align-${alignType}`">
        ...
        <el-form ref="formRef" label-position="left" :model="form" :rules="rules" @submit.native.prevent>
<!--          <div class="title">hello !</div>-->
          <div class="title-tips">{{ translate('欢迎来到') }} {{ title }}</div>

          <el-form-item prop="tenantId">
            <el-input v-model.trim="form.tenantId" v-focus clearable :placeholder="translate('请输入租户ID')" type="text">
              <template #prefix>
                <vab-icon icon="building-2-line" />
              </template>
            </el-input>
          </el-form-item>
...
const behaviorCaptcha = ref<InstanceType<typeof BehaviorCaptcha>>()
...
const validateTenantId = (rule: any, value: any, callback: any) => {
  if ('' === value) callback(new Error(translate('租户不能为空')))
  else callback()
}
...
const handleLogin = async () => {
  ...
  formRef.value.validate(async (valid: boolean) => {
    if (!valid) return
    // 行为验证码模式，打开弹窗，直接终止，不再走doLogin
    if (captchaMode && captchaType === 'behavior') {
      try {
        // 调用打开弹窗
        behaviorCaptcha.value?.open()
      } catch (err){
...
/**
 * 行为验证码验证成功回调（核心 handleBehaviorSuccess）
 * @param res 验证码返回票据 { ticket, randstr }
 */
const handleBehaviorSuccess = async (result: { ticket: string; key: string }) => {
  console.log("handleBehaviorSuccess")
  console.log(result)
  // 把验证码票据存入表单
  form.code = result.ticket
  form.key = result.key
  await doLogin();
}
const doLogin = async () => {
